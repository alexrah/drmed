<?php
						...
						query_posts($query); //get the results
							...
							$terms = get_the_terms(get_the_ID(), 'specialist');
												...
												if (is_array($terms)) {
												 ...
												 $value=get_the_title(get_the_id()).' : '.implode(', ',$terms_slug);
									echo '<option value="'.get_the_title(get_the_id()).' : '.implode(', ',$terms_slug).'"', $meta == $value ? ' selected="selected"' : '', ' >'.get_the_title(get_the_id()).' : '.implode(', ',$terms_slug).'</option>';
						endwhile;
								else :
								$out.='<option>Consult Specialist</option>';
								endif; 
							wp_reset_query();							
							echo '</select></div>';
							break;
				...
				if($_POST['status'])
	{
		...
		$placeholders = array('[contact_name]','[appointment_date]','[appointment_time]','[specialist_name]','[appointment_status]');
		$values = array(get_the_title($post_id),$_POST['dateselect'],$_POST['appointmenttime'],$_POST['specialist'],$_POST['status']);	
		...
		$statussubject=get_option('atp_statussubject');
			...
			$values = array($_POST['specialist'],$post_id);
			$status_email_subject = str_replace($placeholders,$values,$statussubject); //replace the placeholders
								
			$aivahBooking_email=$_POST['email'];
			
		$headers = 'From: ' . $_POST['specialist'] . ' appointment <' . get_option('atp_appointmentemail') . '>' . "\r\n\\";
