...
function optionsframework_add_admin() {
	...
	if(function_exists('add_object_page')) {
		$icon = THEME_URI . '/framework/admin/images/at_icon.jpg'; // Icon for theme admin menu
	...
	function optionsframework_options_page(){
		...
		?>
					...
					<div class="save_bar_top">
						<img style="display:none" src="<?php echo get_stylesheet_directory_uri(); ?>/framework/admin/images/ajax-loader.gif" class="ajax-loading-img ajax-loading-img-bottom" alt="saving..." />
						<input type="submit" value="Save All Changes" class="button-primary" />
			</form>
					...
					<!-- /credits bar -->
					<div class="credits">
					<p>&bull; Framework <a href="http://www.aivahthemes.com/" target="_blank">AivahThemes</a> &bull; Icons <a href="http://www.aivah.com" target="_blank">Aivah</a></p>
					</div>
	...
	function atp_admin_head() {
		...
		jQuery(document).ready(function($){
			...
			foreach($color_pickers_arr as $color_picker) {
				...
				jQuery('#<?php echo $option_id; ?>_picker').css('backgroundColor', '<?php echo $color; ?>');
				jQuery('#<?php echo $option_id; ?>').ColorPicker({
				    onSubmit: function(hsb, hex, rgb) {
		            	jQuery('#<?php echo $option_id; ?>').val('#'+hex);
		            },
		            ...
		            onChange: function (hsb, hex, rgb) {
		            	jQuery('#<?php echo $option_id; ?>_picker').css({'backgroundColor':'#'+hex, 'backgroundImage': 'none', 'borderColor':'#'+hex});
		            	jQuery('#<?php echo $option_id; ?>').prev('input').attr('value', '#'+hex);
						jQuery('#<?php echo $option_id; ?>').val('#'+hex);
		            }
				})	
  		        .bind('keyup', function(){
	...
	function optionsframework_machine($options) {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'subsection':
						$default = $value['name'];
						$output .= '<div class="heading"><strong>'.$default.'</strong>';
						$output .= '<p>'. $explain_value .'</p></div>';
						break;
						...
						if (get_option('atp_social_bookmark') != '') {
							...
							for($i=0;$i<count($sys_social_items);$i++) {
								...
								foreach ( $socialimages_select as $key => $values) {
									...
									$output .= '<option '.$selected.' style='.'background:url('.get_template_directory_uri().'/images/sociables/'.$values.') no-repeat;'.' >'.$values.'</option>';
									$selected ="";
								}
								$output .= '</select>';	
								$output .= '</td>';
				...
				case "typography":
						...
						$google_fonts = array(	
							...
							array( 	'name' => "Open Sans Condensed",'variant' => ':300'),								
				...
				case "heading":
					...
					$menu .= '<li><a title="'.  $value['name'] .'" href="#'.  $jquery_click_hook  .'"><img src="' .$value['icon']. '" height="16" alt=""/> '.  $value['name'] .'</a></li>';
