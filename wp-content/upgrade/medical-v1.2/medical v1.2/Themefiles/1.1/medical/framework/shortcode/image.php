<?php
	...
	function sys_image($atts, $content = null) {
		...
		$content='<figure><img class="'.$class.'" src="'.get_template_directory_uri().'/timthumb.php?src='.$src.'&amp;w='.$width.'&amp;h='.$height.'&amp;zc=1&amp;q=100"   /></figure>';
		}else{ 
		...
		if($lightbox=="true") {
			...
			$out.='<div class="thumb '.($align?' align'.$align:'').'" ><a  '.$target.''.$rel.' '.($no_link? ' class="'.$no_link.'"':'').' title="'.$title.'" href="'.$link.'">' . $content;
			if($caption){
			...
			$out.='<div class="thumb '.($align?' align'.$align:'').'" >' . $content;
			if($caption){
			$out.='<span class="image_caption">'.$caption.'</span>';
	...
	function sys_photoframe( $atts, $content = null ) {
		...
		$out = '<div class="photoframe '.($align?' align'.$align:'').'" '.$framewidth.'><figure>';
