<?php
	...
	class atpgenerator {
		...
		//--------------------------------------------------------
		function skincss() {
			$filename = get_template_directory().'/css/skin.css';

			// Let's make sure the file exists and is writable first.
			if (is_writable($filename)) {
				$handle  = fopen($filename, 'w+');
				$csscontent = include_once( get_template_directory().'/css/skin.php');
		...
		function atp_secondary_menu() {
			if (has_nav_menu( 'secondary-menu' ) ) {
				...
				echo '<li>Go to WP-admin Appearance->Menus and assign menu location</li>';
				echo '</ul>';
			}
		}
			...
			global $post;
				...
				$out .='<ul class="slides">';
					...
					foreach($images as $image) {
						...
						$out .='<li>';
						$out.='<figure><a>';
						$out .= atp_resize('',$full_attachment['0'],$width,$height,'imgborder','');
						$out.='</a></figure>';
						$out .='</li>';
					}//loop ends
					$out .='</ul>';
		...
		function subheader($postid){
			...
			if (is_page() || (is_single()) || (is_singular('doctor')) || (is_front_page() && $post_id != NULL) || (is_home() && $post_id != NULL)){
			...
			if(is_singular( 'doctor' ) ){
				$title = $page->post_title;
				$title.=' : ';
					$terms = get_the_terms( $postid,'specialist');
			...
			$out = '<div id="subheader" '.$subheader_props.'>';
			if(get_option('atp_teaser') != 'disable' ) {
				if($sub_option!="disable"){
					$out .= '<div class="inner">';
					$out .= '<div class="subheaderright">';
						if(isset($title)) {
		...
		function twitter_parse_cache_feed($username,$limit,$encode_utf8 = false) {
			...
			if ($username == '') {
				...
				if ( is_wp_error($messages) ) return "Error fetching Flickr RSS feed".$messages->get_error_message();
		...
		function my_breadcrumb() {
			breadcrumbs_plus(array(
				...
				'echo'		=> true,
				'singular_portfolio_taxonomy'	=> 'portfolio_cat'
			));
		}
			...
			<div id="entry-author-info">
				<div class="authorbg_content">
					<div id="author-avatar">
						<?php echo get_avatar(get_the_author_meta('email'), $size = '80', $default = get_template_directory_uri() . '/images/default_gravatar.gif' ); ?>
					</div>
					<div id="author-description">
						<h5><?php echo get_option('atp_authortxt') ? get_option('atp_authortxt'):'Author';?> : <?php the_author_posts_link(); ?></h5>
		...
		if ($tags) {
			...
			if( $my_query->have_posts() ) { 
				...
				while ($my_query->have_posts()) {
					...
					<?php echo atp_resize($post_id,'','120','70','imgborder',''); ?>
					</a>
					<?php }	else { } ?>
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID() ?>">
		<div id="comment-<?php comment_ID(); ?>" class="comment_wrap">
			<div class="comment-author">
				<?php echo get_avatar($comment,$size='40',$default=$default = $homeurl . '/images/default_gravatar.gif' ); ?>
				<div class="comment-meta">
					<?php printf( '<cite class="fn">%s</cite>', get_comment_author_link()) ?>
					<?php echo get_comment_date(); ?> at <?php comment_time('g:i a'); ?>
