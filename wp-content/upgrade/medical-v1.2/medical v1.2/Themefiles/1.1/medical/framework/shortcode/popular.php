<?php
	...
	function sys_popular_posts ($atts, $content = null) {
		...
		foreach($popular_posts as $post) {
			if($post) {
			$pid = get_post_thumbnail_id($post->ID); echo $posts->ID;
			$ar = wp_get_attachment_image_src( $pid , array(9999,9999) );
				...
				if($thumb == "true"){
					...
					if($thumbs){
						$out.=atp_resize('',$ar['0'],'60','60','','');
						
					}else{
						$out .= '<img class="img_border" src="'. get_template_directory_uri().'/images/no-image.jpg" title="'.$post->post_title.'"  alt=""  />';
					}
					$out .= '</a></figure>';
				}
