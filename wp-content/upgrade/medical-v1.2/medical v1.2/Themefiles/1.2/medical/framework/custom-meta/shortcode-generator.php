<?php
	...
	class atp_shortcode {
		...
		function add() {
			...
			add_meta_box('ShortcodeGenId', 'Shortcodes Generator', array(&$this, 'ShortcodeGenForm'), 'doctor', 'normal','high');
		}
		...
		function typeeditor($type,$id,$atpoptions,$name,$desc,$info,$std,$inputsize) {
			...
			switch ($type) {
				...
				case 'color':
						...
						echo '<td><div class="meta_page_selectwrap"><input class="color" type="color" name="', $id, '" id="', $id, '" value="', $std, '" size="', $inputsize, '" /><div id="', $id, '" class="colorSelector"><div style="background-color: #0000ff"></div></div></div> <span class="desc">', $desc,'</span></td>';
						break;
				case 'text':
						$inputsize = isset($inputsize) ? $inputsize : '10';
						echo '<th scope="row">',$name,'<span class="info">',$info,'</span></th>';
